...
def one_hot_segments(start_indices, end_indices, num_samples):
  ...
  This allows start and end timestamps to be mapped to the same index2.html and the
  ...
  # create a matrix of the index2.html at each row with a column per segment.
  indices = tf.to_int64(
      tf.tile(
...
def timepoint_classification_target(segments, segment_classes, num_classes):
  ...
  Args:
    ...
    segment_classes: a [num_segments] tensor with the class index2.html of each
      segment.
